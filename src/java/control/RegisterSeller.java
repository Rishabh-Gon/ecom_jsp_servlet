...
public class RegisterSeller extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String re_password=request.getParameter("seller_re_password");
        String error = null;
        HttpSession session = request.getSession();
        String getmessage=SellerDAO.ValidSeller(name, mail, password, re_password);
        System.out.println(getmessage);
        session.setAttribute("getmessage", getmessage);
        response.sendRedirect("register_seller.jsp");
    }
